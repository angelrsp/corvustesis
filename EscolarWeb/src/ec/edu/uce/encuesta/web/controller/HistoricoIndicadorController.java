...
public class HistoricoIndicadorController extends SelectItemController implements Serializable {
	...
	public HistoricoIndicadorController() {
...
//					rc.execute("PF('dlgValorReporte').show();");
	...
	private void createMeterGaugeModel(final IndicadorDTO ind) {  
		List<Number> intervals = new ArrayList<Number>(){{
			add(ind.getIndMinimo());  
			add(ind.getIndMaximo());
        }};  
 	   ...
 	   meterGaugeModel.setTitle(ind.getIndNombre());
 	   meterGaugeModel.setGaugeLabel("Resultado");
    }  	
	...
	private void createChartLinePatern(List<IndicadorDTO> list)
	{
		...
		{
			data1.set(ind.getIndNombre(), ind.getIndValorActual());
		}
...
//		for(IndicadorDTO ind:list)
//		{
//			pieModel.set(ind.getIndNombre()+" "+ind.getIndPeso(),  Double.valueOf(ind.getIndPeso()));
//		}
//	 } 
	...
	public void agregarValor()
	{
		try {
			...
			if(getHistoricoIndicadorDTO().getHinValor().doubleValue()>getHistoricoIndicadorDTO().getIndIndicador().getIndMaximo().doubleValue())
			{
				JsfUtil.addErrorMessage("El valor no puede superar valor Maximo");
				return;
			}
