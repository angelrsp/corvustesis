...
public class PremioCiespalDAOImpl extends AbstractFacadeImpl<PremioCiespalDTO> implements PremioCiespalDAO{
	...
	private static final Logger logger = LoggerFactory.getLogger(PremioCiespalDAOImpl.class);
	...
	@Override
	public List<PremioCiespalDTO> getByAnd(PremioCiespalDTO objetoDTO) throws CorvustecException
	{
		CriteriaBuilder cb;
		CriteriaQuery<PremioCiespalDTO> cq;
		Root<PremioCiespalDTO> from;
		List<PremioCiespalDTO> list;
		Predicate predicate;
		...
		try{
			...
			cq=cb.createQuery(PremioCiespalDTO.class);
			
			from= cq.from(PremioCiespalDTO.class);
	        ...
	        for(Field f : fields){
	            fieldName = f.getName();
				if(!fieldName.equals("serialVersionUID")&&!fieldName.equals("pciCount"))
				{
			...
			TypedQuery<PremioCiespalDTO> tq=entityManager.createQuery(cq);
	...
	public int count(Object pais) throws CorvustecException
	{
		...
		try{
			...
			cq.multiselect(cb.count(from.get("pciCodigo")));
			
			predicateList=new ArrayList<Predicate>();
			if(pais!=null)
			{
				predicate=cb.equal(from.get("pciPais"),pais);
				predicateList.add(predicate);
			}
			...
			if(!list.isEmpty())
				return (int)(long)list.get(0).getPciCount();
			else
				return 0;
