...
/**
 * <p>Implementación de la política de la AGE (disponible en 
...
*/	
	...
	/**
	 * Comprueba si la firma contiene:
	 * 	- SigningTime
	 *  - SigningCertificate
	 *  - DataObjectFormat
	 * @param signatureNode Elemento que es la firma con la política que se valida
	...
	protected void checkNodes(final Element signatureNode, final ResultadoValidacion rs) throws PolicyException {
		...
		/*for (DatosNodosFirmados nodo : nodos) {
			if (nodo.isSignInternal()) {
				node = UtilidadTratarNodo.getElementById(signatureNode, nodo.getId());
				if ("SigningTime".equals(node.getLocalName())) {
					hasTime = true;
				} else if ("SigningCertificate".equals(node.getLocalName())) {
					hasCert = true;
			...
			if ((roles != null) && (roles.size() > 0)) {
				...
				while (it.hasNext()) {
					...
					if (!role.matches(MATCH_ROLES)) {
						throw new PolicyException(I18N.getLocalMessage(ConstantsFacturaePolicy.I18N_POLICY_FACTURAE_23));
					}
				}
			}
		}
		else {
			throw new PolicyException(I18N.getLocalMessage(ConstantsFacturaePolicy.I18N_POLICY_FACTURAE_22));
		}*/
	}
