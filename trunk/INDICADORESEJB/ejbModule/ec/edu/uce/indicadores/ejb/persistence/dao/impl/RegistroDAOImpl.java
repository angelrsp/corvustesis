...
public class RegistroDAOImpl extends AbstractFacadeImpl<RegistroDTO> implements RegistroDAO{
	...
	public List<RegistroDTO> getAll(RepresentanteLegalDTO representanteLegalDTO,IesDTO iesDTO) throws IndicadoresException
	{
		...
		Path<RepresentanteLegalDTO> join1=from.join("indIndicador");
		Path<IesDTO> join2=from.join("indIndicador");
		
		cq.where(cb.and(cb.equal(join1.get("indCodigo"), representanteLegalDTO.getRleCodigo()),cb.equal(join2.get(""), iesDTO.getIesCodigo())));
